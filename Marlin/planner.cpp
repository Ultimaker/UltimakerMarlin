...
uint32_t minsegmenttime=DEFAULT_MINSEGMENTTIME;
...
uint32_t max_acceleration_units_per_sq_second[NUM_AXIS] = DEFAULT_MAX_ACCELERATION; // Use M201 to override by software
...
{
  extern uint8_t  planner_max;
  uint8_t         used = moves_planned() + 1;
  ...
  uint32_t segment_time = lround(1000000.0/inverse_second);
  ...
  block->acceleration_rate = (int32_t)((float)block->acceleration_st * (16777216.0 / (F_CPU / 8.0)));
